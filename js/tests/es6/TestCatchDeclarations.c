/* Wide catch declarations, collection, XDR and source round trips.
...
#define CHECK(x) do{++checks;if(!(x)){fprintf(stderr,"CATCH-DECLARATIONS-NATIVE FAIL %u\n",checks);goto out;}}while(0)
int main(void){
 ...
 program=(char*)malloc(1600000);CHECK(program);pos+=(size_t)sprintf(program+pos,"'use strict';var sink;");for(i=0;i<66000;i++)pos+=(size_t)sprintf(program+pos,"sink='unique%u';\n",i);
 strcpy(program+pos,"caughtName='prior';function captured(){return caughtName}try{throw 2}catch(caughtName){var caughtName='inner'}collect();captured()==='prior'&&caughtName==='prior';");
 script=JS_CompileScript(cx,global,program,strlen(program),"wide-catch",1);CHECK(script&&script->atomMap.length>65535&&(scriptObject=JS_NewScriptObject(cx,script))!=NULL);
 
 source=JS_DecompileScript(cx,script,"wide-catch",0);CHECK(source);
 ...
 printf("CATCH-DECLARATIONS-NATIVE PASS checks=%u\n",checks);status=0;
