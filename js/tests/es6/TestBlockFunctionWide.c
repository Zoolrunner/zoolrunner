/* Wide strict block functions, lexical capture, collection and cache/source round trips.
...
#define CHECK(x) do{++checks;if(!(x)){fprintf(stderr,"BLOCK-FUNCTION-WIDE FAIL %u\n",checks);goto out;}}while(0)
int main(void){
 ...
 program=(char*)malloc(1600000);CHECK(program);pos+=(size_t)sprintf(program+pos,"(function wide(){'use strict';var sink;");for(i=0;i<66000;i++)pos+=(size_t)sprintf(program+pos,"sink='unique%u';\n",i);
 strcpy(program+pos,"var captured;{let value=7; captured=f; if(f()!==7)return false;function f(){collect();return value}}if(typeof f!=='undefined')return false;var switched;switch(1){case 1:switched=g();break;default:function g(){return 8}}if(switched!==8||typeof g!=='undefined')return false;collect();return verifyWide(wide)&&captured()===7})()");
 ...
 printf("BLOCK-FUNCTION-WIDE PASS checks=%u\n",checks);status=0;
