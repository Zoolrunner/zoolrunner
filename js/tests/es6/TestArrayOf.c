/* Array.of defaults to its defining realm across classic JSAPI globals.
...
static JSBool Evaluate(JSContext *cx, JSObject *global, const char *source)
{
    ...
    return JS_EvaluateScript(cx, global, source, strlen(source),
                             "array-of-embedding", 1, &result) && result == JSVAL_TRUE;
}
#define CHECK(c) do { ++checks; if (!(c)) { fprintf(stderr,"FAIL Array.of embedding %u\n",checks); goto out; } } while (0)
int main(void)
{
    ...
    CHECK(JS_GetProperty(other, JSVAL_TO_OBJECT(array), "of", &method));
    CHECK(JS_GetProperty(other, JSVAL_TO_OBJECT(array), "prototype", &proto));
    CHECK(JS_DefineProperty(cx, global, "foreignOf", method, NULL, NULL, 0));
    ...
    JS_GC(cx);
    CHECK(Evaluate(cx, global, "Object.getPrototypeOf(foreignOf.call(null,1))===foreignPrototype"));
    CHECK(Evaluate(cx, global, "Object.getPrototypeOf(foreignOf.call(Array,1))===Array.prototype"));
    JS_SetVersion(cx, JSVERSION_DEFAULT);
    CHECK(Evaluate(cx, global, "Object.getPrototypeOf(foreignOf.call(null,1))===foreignPrototype"));
    JS_GC(cx);
    CHECK(Evaluate(cx, global, "foreignOf.call(null,1,2).join()==='1,2'"));
    printf("ES6-ARRAY-OF-EMBEDDING checks=%u failures=0\n", checks);
    status = 0;
  out:
