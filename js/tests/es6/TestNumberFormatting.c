/* Number formatting preserves standard and explicit legacy editions.
...
#define CHECK(c) do{++checks;if(!(c)){fprintf(stderr,"FAIL number formatting check %u\n",checks);goto out;}}while(0)
int main(void)
{
    ...
    uint32 length;
    int status=1;
    const char *program="function modernNumber(){var caught=false;try{(1).toString(4294967298);}catch(e){caught=e instanceof RangeError;}return caught&&(100).toExponential()==='1e+2'&&NaN.toExponential(Infinity)==='NaN';}modernNumber()";
    const char *initialized="function convertedNumber(){var count=0;var s=Infinity.toPrecision({valueOf:function(){++count;return Infinity;}});return s==='Infinity'&&count===1;}convertedNumber()";
    const char *legacy="function legacyNumber(){var caught=false;try{NaN.toExponential(Infinity);}catch(e){caught=e instanceof RangeError;}return caught&&(100).toExponential()==='1.00e+2'&&(10).toString(4294967298)==='1010';}legacyNumber()&&modernNumber()";
    if(!rt)return 1;
    cx=JS_NewContext(rt,8192);if(!cx){JS_DestroyRuntime(rt);return 1;}
    JS_BeginRequest(cx);
    JS_AddNamedRoot(cx,&global,"number formatting global");JS_AddNamedRoot(cx,&scriptObject,"number formatting script");
    JS_AddNamedRoot(cx,&decodedObject,"number formatting decoded");JS_AddNamedRoot(cx,&source,"number formatting source");
    ...
    script=JS_CompileScript(cx,global,program,strlen(program),"number-formatting",1);
    ...
    source=JS_DecompileScript(cx,decoded,"number-formatting-source",0);CHECK(source);
    JS_SetVersion(cx,JSVERSION_ECMA_2015);
    ...
    CHECK(JS_EvaluateScript(cx,global,"legacyNumber()&&modernNumber()",strlen("legacyNumber()&&modernNumber()"),"cross-edition",1,&result)&&result==JSVAL_TRUE);
    JS_SetVersion(cx,JSVERSION_DEFAULT);
    CHECK(JS_EvaluateScript(cx,global,"convertedNumber()",strlen("convertedNumber()"),"default",1,&result)&&result==JSVAL_TRUE);
    printf("ES6-NUMBER-FORMATTING checks=%u failures=0\n",checks);status=0;
out:
