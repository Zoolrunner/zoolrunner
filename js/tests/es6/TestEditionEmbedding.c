...
int main(void)
{
    ...
    const char *modern =
        ...
        "immutable && "
        "(function(){function C(){return {target:new.target,"
        "direct:eval('new.target'),strict:eval('\"use strict\";new.target')};}"
        "function D(){}var c=Reflect.construct(C,[],D);"
        "return c.target===D && c.direct===D && c.strict===D && "
        "C().target===undefined && new C().target===C;})() && "
        "(function(){var order=[],o={},key={toString:function(){order.push('key');return 'x';}};"
        "o[key]=(order.push('rhs'),3);o[key]+=(order.push('rhs'),2);"
        "try{null.x=(order.push('bad'),1);}catch(e){"
        "return e instanceof TypeError && order.join()==='key,rhs,key,rhs' && o.x===5;}"
        "return false;})() && "
        ...
        "Object.defineProperty(g,'editionBindingProbe',{value:3,writable:true,configurable:true});"
        "try{editionBindingProbe+=(delete g.editionBindingProbe,2);"
        "if(g.editionBindingProbe!==5)return false;delete g.editionBindingProbe;"
        "try{editionBindingProbe=(g.editionBindingProbe=1,2);}catch(e){"
        "return e instanceof ReferenceError && g.editionBindingProbe===1;}return false;"
        "}finally{delete g.editionBindingProbe;}})() && "
