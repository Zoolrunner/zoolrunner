/* Wide parenthesized default targets, names, collection and cache/source round trips.
...
#define CHECK(x) do{++checks;if(!(x)){fprintf(stderr,"ASSIGNMENT-TARGET-WIDE FAIL %u\n",checks);goto out;}}while(0)
int main(void){
 ...
 strcpy(program+pos,"var local;[(local)=function(){}]=[];if(local.name!=='')return false;[(wideGlobalTarget)=function(){}]=[];if(wideGlobalTarget.name!=='')return false;({x:(wideOtherTarget)=function(){}}={});if(wideOtherTarget.name!=='')return false;collect();return verifyWide(wide)})()");
 script=JS_CompileScript(cx,global,program,strlen(program),"wide-assignment-targets",1);CHECK(script&&(scriptObject=JS_NewScriptObject(cx,script))!=NULL);
 
 source=JS_DecompileScript(cx,script,"wide-assignment-targets",0);CHECK(source);
 ...
 printf("ASSIGNMENT-TARGET-WIDE PASS checks=%u\n",checks);status=0;
