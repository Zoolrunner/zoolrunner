/* Parenthesized default targets preserve editions and cache/source round trips.
...
#define CHECK(c) do{++checks;if(!(c)){fprintf(stderr,"FAIL assignment targets check %u\n",checks);goto out;}}while(0)
int main(void)
{
    ...
    uint32 length;
    int status=1;
    const char *program="function modernTargets(){var a;[(a)=function(){}]=[];return a.name==='';}modernTargets()";
    const char *initialized="function modernObjectTarget(){var a;({x:(a)=function(){}}={});return a.name==='';}modernObjectTarget()";
    const char *legacy="function legacyTarget(){var a;([a])=[7];return a===7;}legacyTarget()&&modernTargets()";
    if(!rt)return 1;
    cx=JS_NewContext(rt,8192);if(!cx){JS_DestroyRuntime(rt);return 1;}
    JS_BeginRequest(cx);
    JS_AddNamedRoot(cx,&global,"assignment targets global");JS_AddNamedRoot(cx,&scriptObject,"assignment targets script");
    JS_AddNamedRoot(cx,&decodedObject,"assignment targets decoded");JS_AddNamedRoot(cx,&source,"assignment targets source");
    ...
    script=JS_CompileScript(cx,global,program,strlen(program),"assignment-targets",1);
    ...
    source=JS_DecompileScript(cx,decoded,"assignment-targets-source",0);CHECK(source);
    JS_SetVersion(cx,JSVERSION_ECMA_2015);
    ...
    CHECK(JS_EvaluateScript(cx,global,"legacyTarget()&&modernTargets()",strlen("legacyTarget()&&modernTargets()"),"cross-edition",1,&result)&&result==JSVAL_TRUE);
    JS_SetVersion(cx,JSVERSION_DEFAULT);
    CHECK(JS_EvaluateScript(cx,global,"modernObjectTarget()",strlen("modernObjectTarget()"),"default",1,&result)&&result==JSVAL_TRUE);
    printf("ES6-ASSIGNMENT-TARGETS checks=%u failures=0\n",checks);status=0;
out:
