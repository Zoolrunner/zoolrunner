...
fixtures = [
    ...
    ('queued-complete', 'strict', {'flags': ['async']}, 'enqueueJob(function() { $DONE(); });', 'pass'),
    ('queued-error', 'strict', {'flags': ['async']}, 'enqueueJob(function() { $DONE(new Error("bad")); });', 'fail'),
    ('queued-duplicate', 'strict', {'flags': ['async']}, '$DONE(); enqueueJob(function() { $DONE(); });', 'fail'),
