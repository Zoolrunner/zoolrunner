/* Multiline HTML close comments preserve saved editions and ASI.
...
#define CHECK(c) do{++checks;if(!(c)){fprintf(stderr,"FAIL HTML comments check %u\n",checks);goto out;}}while(0)
int main(void)
{
    ...
    uint32 length;
    int status=1;
    const char *program="function modernComment(){return eval('0/*\\n*/--> comment\\n7')===7;}modernComment()";
    const char *initialized="function modernASI(){return/*\n*/--> comment\n9;}modernASI()===undefined";
    const char *legacy="function legacyComment(){try{eval('0/*\\n*/--> comment\\n7');}catch(e){return e instanceof SyntaxError;}return false;}legacyComment()&&modernComment()";
    if(!rt)return 1;
    cx=JS_NewContext(rt,8192);if(!cx){JS_DestroyRuntime(rt);return 1;}
    JS_BeginRequest(cx);
    JS_AddNamedRoot(cx,&global,"HTML comments global");JS_AddNamedRoot(cx,&scriptObject,"HTML comments script");
    JS_AddNamedRoot(cx,&decodedObject,"HTML comments decoded");JS_AddNamedRoot(cx,&source,"HTML comments source");
    ...
    script=JS_CompileScript(cx,global,program,strlen(program),"html-comments",1);
    ...
    source=JS_DecompileScript(cx,decoded,"html-comments-source",0);CHECK(source);
    JS_SetVersion(cx,JSVERSION_ECMA_2015);
    ...
    CHECK(JS_EvaluateScript(cx,global,"legacyComment()&&modernComment()",strlen("legacyComment()&&modernComment()"),"cross-edition",1,&result)&&result==JSVAL_TRUE);
    JS_SetVersion(cx,JSVERSION_DEFAULT);
    CHECK(JS_EvaluateScript(cx,global,"modernASI()===undefined",strlen("modernASI()===undefined"),"default",1,&result)&&result==JSVAL_TRUE);
    printf("ES6-HTML-COMMENTS checks=%u failures=0\n",checks);status=0;
out:
