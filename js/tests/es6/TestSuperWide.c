/* Wide super operands, captured references, collection and cache/source round trips.
...
#define CHECK(x) do{++checks;if(!(x)){fprintf(stderr,"SUPER-WIDE FAIL %u\n",checks);goto out;}}while(0)
int main(void){
 ...
 program=(char*)malloc(1600000);CHECK(program);pos+=(size_t)sprintf(program+pos,"(function(){var base={get x(){collect();return this.saved},set x(v){collect();this.saved=v}},o={saved:3,m(){var sink;");for(i=0;i<66000;i++)pos+=(size_t)sprintf(program+pos,"sink='unique%u';\n",i);
 strcpy(program+pos,"var old=super.x++;super.x+=2;[super.x]=[super.x+1];var arrow=()=>super.x;for(super.x of [9,10]){}for(super.x in {a:1}){}super.x=10;return old===3&&arrow()===10&&eval('super.x')===10;}};Object.setPrototypeOf(o,base);return verifyWide(o.m)&&o.m()})()");
 script=JS_CompileScript(cx,global,program,strlen(program),"wide-super",1);CHECK(script&&(scriptObject=JS_NewScriptObject(cx,script))!=NULL);
 
 source=JS_DecompileScript(cx,script,"wide-super",0);CHECK(source);
 ...
 printf("SUPER-WIDE PASS checks=%u\n",checks);status=0;
