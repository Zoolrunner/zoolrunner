/* Global declaration preflight through native callbacks, GC and XDR.
...
{fprintf(stderr,"global declarations: %s\n",message);}
#define CHECK(v) do {++checks;if(!(v)){fprintf(stderr,"FAIL global declarations check %u\n",checks);goto out;}}while(0)
int main(void)
{
    ...
    uint32 length;
    int status=1;
    const char *program="var gdOne=1;function gdNative(){return gdOne}gdNative()===1";
    ...
    printf("GLOBAL-DECLARATIONS-EMBEDDING PASS checks=%u\n",checks);status=0;
  out:
