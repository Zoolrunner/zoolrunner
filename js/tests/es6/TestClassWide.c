/* Wide class operands, derived constructors, collection and cache/source round trips.
...
#define CHECK(x) do{++checks;if(!(x)){fprintf(stderr,"CLASS-WIDE FAIL %u\n",checks);goto out;}}while(0)
int main(void){
 ...
 strcpy(program+pos,"class Base {constructor(x){collect();this.x=x}m(){return this.x}} class Derived extends Base {constructor(x){var f=()=>super(x);f();collect()}m(){collect();return super.m()+1}static test(){return this.name}} var o=new Derived(8);var text=Derived.toString();collect();var Copy=eval('('+text+')');return verifyWide(wide)&&o.m()===9&&new Copy(9).m()===10&&Copy.test()==='Derived'&&text.indexOf('class Derived')===0})()");
 script=JS_CompileScript(cx,global,program,strlen(program),"wide-class",1);CHECK(script&&(scriptObject=JS_NewScriptObject(cx,script))!=NULL);
 
 source=JS_DecompileScript(cx,script,"wide-class",0);CHECK(source);
 ...
 printf("CLASS-WIDE PASS checks=%u\n",checks);status=0;
