...
| Windows x86 | The September 16 Suite package passes 17 regression groups in NT 4.0, Me, and 2000 guests. Cross-build and package audits are documented separately. | Windows 95 import blockers and runtime validation remain open. The four-application pipeline is not yet recorded as a complete local pass. |
...
| Linux i686 / x86_64 | Both GTK2 and Xlib have profiles and CI jobs for all four applications. An isolated x86_64 GTK2 Suite rebuild passes the packaged runtime checks, including Venkman lifecycle and ChatZilla, with the GCC compatibility settings. A fresh x86_64 Xlib Suite build compiles and packages. | Xlib lifecycle testing still times out. Full fresh-build and runtime matrix validation remains in progress; see the [Linux status](build/linux/README.md). |
